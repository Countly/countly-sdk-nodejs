...
var cc = {
     ...
     */
    isResponseValid: function isResponseValid(res) {
        // status code and response format check
        if (typeof res === "object" && res !== null && res.statusCode >= 200 && res.statusCode < 300) {
            // JSON format check
            try {
                var contentType = JSON.stringify(res.headers);
