...
var cc = {
     ...
     * @param {Object} res - http incoming message
     * @param {Object} str - response from server, ideally must be: {"result":"Success"} or should contain at least result field
     * @returns {Boolean} - returns true if response passes the tests 
     */
    isResponseValid: function isResponseValid(res, str) {
        // status code and response format check
        if (res.statusCode >= 200 && res.statusCode < 300) {
            if (typeof str === "object" && str !== null && str.result) {
                this.log("Valid HTTP response.");
                return true;
            }
            else {
                try {
                    ...
                    this.log("Http response is in wrong format.", e);
                    return false;
                }
            }
        }
        this.log("Http response is not valid.");
        return false;
    }
};
