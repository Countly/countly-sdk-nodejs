...
function CountlyBulk(conf) {
    ...
    if (!conf.app_key) {
        log(logLevelEnums.WARNING, "app_key is missing");
        return;
    }

    if (!conf.url) {
        log(logLevelEnums.WARNING, "url is missing");
        return;
    }
    ...
    **/
    this.add_request = function(query) {
        if (cluster.isMaster) {
            if (!query.device_id) {
                log(logLevelEnums.WARNING, "add_request, device_id is missing", query);
                return;
            }
            ...
            if ((query.timestamp + "").length !== 13 && (query.timestamp + "").length !== 10) {
                log(logLevelEnums.WARNING, "add_request, incorrect timestamp format", query);
            }
            ...
            log(logLevelEnums.INFO, "add_request, adding request", query);
    ...
    this.add_bulk_request = function(requests) {
        if (cluster.isMaster) {
            ...
            for (var i in requests) {
                query = requests[i];
                if (!query.device_id) {
                    log(logLevelEnums.WARNING, "add_bulk_request, device_id is missing", query);
                    return;
                }
                ...
                if ((query.timestamp + "").length !== 13 && (query.timestamp + "").length !== 10) {
                    log(logLevelEnums.WARNING, "add_bulk_request, incorrect timestamp format", query);
                }
                ...
                query.hour = query.hour || date.getHours();
                query.dow = query.dow || date.getDay();
                log(logLevelEnums.INFO, "add_bulk_request, adding bulk request", query);
                requestQueue.push(query);
            }
            storeSet("cly_req_queue", requestQueue);
        }
    ...
    **/
    this.add_event = function(device_id, event) {
        if (!device_id) {
            log(logLevelEnums.WARNING, "add_event, device_id is missing");
            return;
        }

        if (!event.key) {
            log(logLevelEnums.WARNING, "add_event, Event must have key property");
            return;
        }
        if (cluster.isMaster) {
            if (!event.count) {
    ...
    */
    function makeRequest(params, callback) {
        try {
            log(logLevelEnums.DEBUG, "Sending HTTP request");
