...
function CountlyBulk(conf) {
    ...
    **/
    this.add_request = function(query) {
        query = truncateObject(query, self.maxKeyLength, self.maxValueSize, self.maxSegmentationValues, "add_request", self.debug);
        if (cluster.isMaster) {
            if (!query.device_id) {
    ...
    **/
    this.add_event = function(device_id, event) {
        ...
        if (cluster.isMaster) {
            ...
            event.key = truncateSingleValue(event.key, self.maxKeyLength, "add_event", self.debug);
            event.segmentation = truncateObject(event.segmentation, self.maxKeyLength, self.maxValueSize, self.maxSegmentationValues, "add_event", self.debug);
    ...
    **/
    this.add_user = function(userConf) {
        userConf = truncateObject(userConf, self.maxKeyLength, self.maxValueSize, self.maxSegmentationValues, "add_user", self.debug);
