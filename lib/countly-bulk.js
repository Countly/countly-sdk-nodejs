...
function CountlyBulk(conf) {
    ...
    if (!conf.app_key) {
        cc.log(cc.logLevelEnums.WARNING, "app_key is missing");
        return;
    }

    if (!conf.url) {
        cc.log(cc.logLevelEnums.WARNING, "url is missing");
        return;
    }
    ...
    **/
    this.add_request = function(query) {
        if (cluster.isMaster) {
            if (!query.device_id) {
                cc.log(cc.logLevelEnums.WARNING, "add_request, device_id is missing", query);
                return;
            }
            ...
            if ((query.timestamp + "").length !== 13 && (query.timestamp + "").length !== 10) {
                cc.log(cc.logLevelEnums.WARNING, "add_request, incorrect timestamp format", query);
            }
    ...
    this.add_bulk_request = function(requests) {
        if (cluster.isMaster) {
            ...
            for (var i in requests) {
                query = requests[i];
                if (!query.device_id) {
                    cc.log(cc.logLevelEnums.WARNING, "add_bulk_request, device_id is missing", query);
                    return;
                }
                ...
                if ((query.timestamp + "").length !== 13 && (query.timestamp + "").length !== 10) {
                    cc.log(cc.logLevelEnums.WARNING, "add_bulk_request, incorrect timestamp format", query);
                }
    ...
    **/
    this.add_event = function(device_id, event) {
        if (!device_id) {
            cc.log(cc.logLevelEnums.WARNING, "add_event, device_id is missing");
            return;
        }

        if (!event.key) {
            cc.log(cc.logLevelEnums.WARNING, "add_event, Event must have key property");
            return;
        }
        if (cluster.isMaster) {
            if (!event.count) {
