...
function CountlyBulk(conf) {
    ...
    if (!conf.app_key) {
        log(logLevelEnums.WARNING, "app_key is missing", conf.debug);
        return;
    }

    if (!conf.url) {
        log(logLevelEnums.WARNING, "url is missing", conf.debug);
        return;
    }
    ...
    **/
    this.add_request = function(query) {
        if (cluster.isMaster) {
            if (!query.device_id) {
                log(logLevelEnums.WARNING, "add_request, device_id is missing", conf.debug, query);
                return;
            }
            ...
            if ((query.timestamp + "").length !== 13 && (query.timestamp + "").length !== 10) {
                log(logLevelEnums.WARNING, "add_request, incorrect timestamp format", conf.debug, query);
            }
            ...
            log(logLevelEnums.INFO, "add_request, adding request", conf.debug, query);
    ...
    this.add_bulk_request = function(requests) {
        if (cluster.isMaster) {
            ...
            for (var i in requests) {
                query = requests[i];
                if (!query.device_id) {
                    log(logLevelEnums.WARNING, "add_bulk_request, device_id is missing", conf.debug, query);
                    return;
                }
                ...
                if ((query.timestamp + "").length !== 13 && (query.timestamp + "").length !== 10) {
                    log(logLevelEnums.WARNING, "add_bulk_request, incorrect timestamp format", conf.debug, query);
                }
                ...
                query.hour = query.hour || date.getHours();
                query.dow = query.dow || date.getDay();
                log(logLevelEnums.INFO, "add_bulk_request, adding bulk request", conf.debug, query);
                requestQueue.push(query);
            }
            storeSet("cly_req_queue", requestQueue);
        }
    ...
    **/
    this.add_event = function(device_id, event) {
        if (!device_id) {
            log(logLevelEnums.WARNING, "add_event, device_id is missing", conf.debug);
            return;
        }

        if (!event.key) {
            log(logLevelEnums.WARNING, "add_event, Event must have key property", conf.debug);
            return;
        }
        if (cluster.isMaster) {
            if (!event.count) {
            ...
            e.dow = date.getDay();
            log(logLevelEnums.INFO, "add_event, Adding event: ", conf.debug, event);
    ...
    */
    function heartBeat() {
        ...
        if (bulkQueue.length > 0 && readyToProcess && getTimestamp() > failTimeout) {
            ...
            log(logLevelEnums.DEBUG, "Processing request", conf.debug, params);
            makeRequest(params, function(err, res) {
                log(logLevelEnums.DEBUG, "Request Finished", conf.debug, res, err);
                if (err) {
    ...
    */
    function makeRequest(params, callback) {
        try {
            log(logLevelEnums.DEBUG, "Sending HTTP request", conf.debug);
            ...
            req.on("error", function(err) {
                log(logLevelEnums.ERROR, "Connection failed.", conf.debug, err);
            ...
            // fallback
            log(logLevelEnums.ERROR, "Failed HTTP request", conf.debug, e);
            if (typeof callback === "function") {
                callback(true, params);
            }
        }
