...
function CountlyBulkUser(conf) {
    ...
    */
    ...
    if (!conf.device_id) {
        log(logLevelEnums.WARNING, "device_id is missing", conf.debug);
        return;
    }

    if (!conf.server) {
        log(logLevelEnums.WARNING, "server instance is missing", conf.debug);
        return;
    }
            ...
            for (var i in groupedFeatures) {
                if (!consents[i]) {
                    if (typeof groupedFeatures[i] === "string") {
                        ...
                        log(logLevelEnums.WARNING, "group_features, Incorrect feature list for " + i + " value: " + groupedFeatures[i], conf.debug);
                    }
                }
                else {
                    log(logLevelEnums.WARNING, "group_features, Feature name " + i + " is already reserved", conf.debug);
                }
            }
        }
        else {
            log(logLevelEnums.WARNING, "group_features, Incorrect features: " + groupedFeatures, conf.debug);
        }
    };
    ...
    */
    this.check_consent = function(feature) {
        ...
        if (consents[feature]) {
            ...
            log(logLevelEnums.WARNING, "check_consent, No feature available for " + feature, conf.debug);
        }
        return false;
    };
    ...
    */
    this.add_consent = function(feature) {
        log(logLevelEnums.INFO, "add_consent, Adding consent for " + feature, conf.debug);
        if (feature.constructor === Array) {
            ...
            log(logLevelEnums.WARNING, "add_consent, No feature available for " + feature, conf.debug);
        }
    };
    ...
    */
    this.remove_consent = function(feature) {
        log(logLevelEnums.INFO, "remove_consent, Removing consent for " + feature, conf.debug);
    ...
    */
    this.remove_consent_internal = function(feature, enforceConsentUpdate) {
        ...
        if (feature.constructor === Array) {
            ...
            log(logLevelEnums.WARNING, "remove_consent, No feature available for " + feature, conf.debug);
        }
    };
    ...
    this.report_feedback = function(feedback, timestamp) {
        if (this.check_consent("star-rating") || this.check_consent("feedback")) {
            if (!feedback.widget_id) {
                log(logLevelEnums.WARNING, "report_feedback, Feedback must contain widget_id property", conf.debug);
                return;
            }
            if (!feedback.rating) {
                log(logLevelEnums.WARNING, "report_feedback, Feedback must contain rating property", conf.debug);
                return;
            }
            if (this.check_consent("events")) {
                ...
                log(logLevelEnums.INFO, "report_feedback, Reporting feedback: ", conf.debug, event);
        ...
        if (this.check_consent("apm")) {
            ...
            for (var i = 0; i < props.length; i++) {
                if (props[i] !== "apm_attr" && typeof trace[props[i]] === "undefined") {
                    log(logLevelEnums.WARNING, "report_trace, APM trace must have a", conf.debug, props[i]);
                    return;
                }
            }
            ...
            log(logLevelEnums.INFO, "report_trace, Adding APM trace: ", conf.debug, e);
            conf.server.add_request(query);
        }
        return this;
    };
