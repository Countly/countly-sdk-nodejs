...
function CountlyBulkUser(conf) {
    ...
    */
    this.check_consent = function(feature) {
        if (!conf.require_consent) {
            ...
            log("Require consent is off. Giving consent for : [ " + feature + " ] feature.");
            return true;
        }
        if (consents[feature] && consents[feature].optin) {
            log("Giving consent for : [ " + feature + " ] feature.");
            return true;
        }
        if (consents[feature] && !consents[feature].optin) {
            log("User is not optin. Consent refused for : [ " + feature + " ] feature.");
            return false;
        }
        else {
            log("No feature available for : [ " + feature + " ] feature.");
        }
        return false;
    };
    ...
    **/
    this.add_event = function(event) {
        log('Trying to add the event: [ ' + event.key + ' ]');
