...
function CountlyBulkUser(conf) {
    ...
    */
    ...
    if (!conf.device_id) {
        log(logLevelEnums.WARNING, "device_id is missing");
        return;
    }

    if (!conf.server) {
        log(logLevelEnums.WARNING, "server instance is missing");
        return;
    }
    ...
    */
    this.group_features = function(groupedFeatures) {
        if (groupedFeatures) {
            for (var i in groupedFeatures) {
                if (!consents[i]) {
                    if (typeof groupedFeatures[i] === "string") {
                        ...
                        log(logLevelEnums.WARNING, "group_features, Incorrect feature list for " + i + " value: " + groupedFeatures[i]);
                    }
                }
                else {
            ...
            log(logLevelEnums.WARNING, "group_features, Incorrect features: " + groupedFeatures);
        }
    };
    ...
    **/
    this.report_feedback = function(feedback, timestamp) {
        if (this.check_consent("star-rating") || this.check_consent("feedback")) {
            ...
            if (this.check_consent("events")) {
                ...
                log(logLevelEnums.INFO, "report_feedback, Reporting feedback: ", event);
    ...
    this.report_trace = function(trace) {
        if (this.check_consent("apm")) {
            ...
            log(logLevelEnums.INFO, "report_trace, Adding APM trace: ", e);
            conf.server.add_request(query);
        }
        return this;
    };
