 ...
 */
    Countly.init = function(conf) {
        if (!inited) {
            ...
            if (Countly.url === "") {
                cc.log(cc.logLevelEnums.WARNING, "Please provide server URL");
            }
            else {
                cc.log("info", "Countly initialized");
                if (cluster.isMaster) {
    ...
    */
    Countly.group_features = function(features) {
        if (features) {
            ...
            cc.log(cc.logLevelEnums.WARNING, "group_features, Incorrect features: " + features);
        }
    };
    ...
    */
    Countly.add_consent = function(feature) {
        cc.log("info", "add_consent, Adding consent for " + feature);
    ...
    */
    Countly.remove_consent = function(feature) {
        cc.log("info", "remove_consent, Removing consent for " + feature);
    ...
    */
    ...
    Countly.begin_session = function(noHeartBeat) {
        if (Countly.check_consent("sessions")) {
            if (!sessionStarted) {
                cc.log("info", "begin_session, Session started");
    ...
    */
    Countly.session_duration = function(sec) {
        if (Countly.check_consent("sessions")) {
            if (sessionStarted) {
                cc.log("info", "session_duration, Session extended", sec);
    ...
    Countly.end_session = function(sec) {
        if (Countly.check_consent("sessions")) {
            if (sessionStarted) {
                ...
                cc.log("info", "end_session, Ending session");
                reportViewDuration();
                sessionStarted = false;
    ...
    Countly.change_id = function(newId, merge) {
        if (cluster.isMaster) {
            if (Countly.device_id !== newId) {
                ...
                storeSet("cly_id", Countly.device_id);
                cc.log("info", "change_id, Changing ID");
                if (merge) {
    ...
    */
    function add_cly_events(event) {
        if (!event.key) {
            cc.log(cc.logLevelEnums.WARNING, "Event must have key property");
            return;
        }
        if (cluster.isMaster) {
            if (!event.count) {
    ...
    **/
    Countly.end_event = function(event) {
        ...
        if (!event.key) {
            cc.log(cc.logLevelEnums.WARNING, "end_event, Event must have key property");
            return;
        }
    ...
    Countly.report_conversion = function(campaign_id, campaign_user_id) {
        if (Countly.check_consent("attribution")) {
            if (campaign_id && campaign_user_id) {
                ...
                cc.log(cc.logLevelEnums.WARNING, "report_conversion, No campaign data found");
            }
        }
    };
    ...
    Countly.report_feedback = function(feedback) {
        if (Countly.check_consent("star-rating") || Countly.check_consent("feedback")) {
            if (!feedback.widget_id) {
                cc.log(cc.logLevelEnums.WARNING, "report_feedback, Feedback must contain widget_id property");
                return;
            }
            if (!feedback.rating) {
                cc.log(cc.logLevelEnums.WARNING, "report_feedback, Feedback must contain rating property");
                return;
            }
            if (Countly.check_consent("events")) {
    ...
    */
    function makeRequest(url, api, params, callback) {
        try {
            cc.log('info', "Sending HTTP request");
