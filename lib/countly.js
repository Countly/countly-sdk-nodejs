    ...
    Countly.init = function(conf) {
        if (!inited) {
            ...
            try {
                ...
                cc.log(cc.logLevelEnums.ERROR, `init, Failed to create the data directory:${ex.stack}`);
            }
    ...
    */
    function makeRequest(url, api, params, callback) {
        try {
            ...
            var req = protocol.request(options, (res) => {
                ...
                res.on("end", () => {
                    cc.log(cc.logLevelEnums.DEBUG, `makeRequest, Server status code: [${res.statusCode}], server response: [${str}]`);
    ...
    var readFile = function(key) {
        ...
        try {
            ...
            cc.log(cc.logLevelEnums.ERROR, `readFile, Failed to parse the file: [${ex.stack}]`);
    ...
    var forceStore = function() {
        for (var i in __data) {
            ...
            try {
                ...
                cc.log(cc.logLevelEnums.ERROR, `forceStore, Saving files failed: [${ex.stack}]`);
            }
        }
    };
