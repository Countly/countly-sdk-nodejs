...
(function() {
    ...
    var __data = {};
    var deviceIdType = cc.deviceIdTypeEnums.SDK_GENERATED;
 ...
 * @param {boolean} [conf.erase_stored_id=false] - set it to true if you want to erase the stored device ID
 ...
 */
    Countly.init = function(conf) {
        if (!inited) {
            ...
            conf.erase_stored_id = conf.erase_stored_id || false;
    ...
    */
    ...
    /**
    *  If an ID is provided sets the device ID  type, or generates new device ID
    * @param {String} storedDeviceId - stored device ID to check
    *  @returns {String} device id
    */
    function assignId(storedDeviceId) {
        if (storedDeviceId) {
            deviceIdType = storeGet("cly_id_type", null);
            return storedDeviceId;
        }

        return cc.generateUUID();
    }
