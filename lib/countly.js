 ...
 */
    Countly.init = function(conf) {
        if (!inited) {
            ...
            try {
                ...
                console.log(ex.stack, logLevelEnums.ERROR);
            }

            if (Countly.url === "") {
                log("Please provide server URL", logLevelEnums.WARNING);
            }
            else {
            ...
            for (var i in features) {
                if (!consents[i]) {
                    if (typeof features[i] === "string") {
                        ...
                        log("Incorrect feature list for " + i + " value: " + features[i], logLevelEnums.WARNING);
                    }
                }
                else {
                    log("Feature name " + i + " is already reserved", logLevelEnums.WARNING);
                }
            }
        }
        else {
            log("Incorrect features: " + features, logLevelEnums.WARNING);
        }
    };
    ...
    */
    Countly.check_consent = function(feature) {
        ...
        if (consents[feature]) {
            ...
            log("No feature available for " + feature, logLevelEnums.WARNING);
        }
        return false;
    };
    ...
    */
    function add_cly_events(event) {
        if (!event.key) {
            log("Event must have key property", logLevelEnums.WARNING);
            return;
        }
        if (cluster.isMaster) {
            if (!event.count) {
            ...
            e.dow = date.getDay();
            log("Adding event: ", event, logLevelEnums.DEBUG);
            eventQueue.push(e);
            storeSet("cly_event", eventQueue);
        }
        else {
    ...
    **/
    Countly.start_event = function(key) {
        if (timedEvents[key]) {
            log("Timed event with key " + key + " already started", logLevelEnums.DEBUG);
            return;
        }
    ...
    **/
    Countly.end_event = function(event) {
        ...
        if (!timedEvents[event.key]) {
            log("Timed event with key " + event.key + " was not started", logLevelEnums.WARNING);
            return;
        }
    ...
    **/
    Countly.user_details = function(user) {
        if (Countly.check_consent("users")) {
            log("Adding userdetails: ", user, logLevelEnums.DEBUG);
    ...
    Countly.report_conversion = function(campaign_id, campaign_user_id) {
        if (Countly.check_consent("attribution")) {
            if (campaign_id && campaign_user_id) {
                ...
                log("No campaign data found", logLevelEnums.WARNING);
            }
        }
    };
    ...
    Countly.report_feedback = function(feedback) {
        if (Countly.check_consent("star-rating") || Countly.check_consent("feedback")) {
            if (!feedback.widget_id) {
                log("Feedback must contain widget_id property", logLevelEnums.WARNING);
                return;
            }
            if (!feedback.rating) {
                log("Feedback must contain rating property", logLevelEnums.WARNING);
                return;
            }
            if (Countly.check_consent("events")) {
                ...
                log("Reporting feedback: ", event, logLevelEnums.DEBUG);
                Countly.add_event(event);
            }
        }
    };
    ...
    **/
    Countly.fetch_remote_config = function(keys, omit_keys, callback) {
        if (Countly.check_consent("remote-config")) {
            ...
            log("Remote config requires explicit consent", logLevelEnums.WARNING);
            if (typeof callback === "function") {
        ...
        if (Countly.check_consent("apm")) {
            ...
            for (var i = 0; i < props.length; i++) {
                if (props[i] !== "apm_attr" && typeof trace[props[i]] === "undefined") {
                    log("APM trace must have a", props[i], logLevelEnums.WARNING);
                    return;
                }
            }
            ...
            log("Adding APM trace: ", e, logLevelEnums.DEBUG);
        }
    };
    ...
    **/
    Countly.request = function(request) {
        if (!request.app_key || !request.device_id) {
            log("app_key or device_id is missing", logLevelEnums.WARNING);
            return;
        }
        if (cluster.isMaster) {
    ...
    */
    function heartBeat() {
        ...
        if (requestQueue.length > 0 && readyToProcess && getTimestamp() > failTimeout) {
            ...
            log("Processing request", params, logLevelEnums.DEBUG);
            makeRequest(Countly.url, apiPath, params, function(err, res) {
                log("Request Finished", res, err, logLevelEnums.DEBUG);
                if (err) {
    ...
    */
    function getMetrics() {
        ...
        log("Got metrics", m, logLevelEnums.DEBUG);
        return m;
    }

    /**
     *  Log data if debug mode is enabled
     *  First arguments must be the main message
     *  Last argument indicates the log level
     */
    function log() {
        if (Countly.debug && typeof console !== "undefined") {
            ...
            var prefix = logLevelEnums.INFO;
            var color = '';
            for (var arg in arguments) {
                if ((arg in logLevelEnums)) {
                    prefix = arg;
                    //change color
                    if (arg === logLevelEnums.ERROR) {
                        //red color
                        color = '\x1b[31m%s\x1b[0m';
                    }
                    else if (arg === logLevelEnums.WARNING) {
                        //yellow
                        color = '\x1b[33m%s\x1b[0m';
                    }
                    else if (arg === logLevelEnums.DEBUG) {
                        //cyan
            ...
            // eslint-disable-next-line no-console
            console.log(color, prefix + (Array.prototype.slice.call(arguments).includes(prefix) ? Array.prototype.slice.call(arguments).slice(0, -1) : Array.prototype.slice.call(arguments).join("\n")));
        }
    }
    // function log() {
    //     if (Countly.debug && typeof console !== "undefined") {
    ...
    */
    function makeRequest(url, api, params, callback) {
        try {
            ...
            req.on("error", function(err) {
                log("Connection failed.", err, logLevelEnums.ERROR);
                if (typeof callback === "function") {
                    callback(true, params);
                }
            });
            ...
            // fallback
            log("Failed HTTP request", e, logLevelEnums.ERROR);
            if (typeof callback === "function") {
                callback(true, params);
            }
        }
    ...
    var writeFile = function(key, value, callback) {
        ...
        fs.writeFile(dir, JSON.stringify(ob), function(err) {
            if (err) {
                // eslint-disable-next-line no-console
                console.log(err, logLevelEnums.ERROR);
            }
            if (typeof callback === "function") {
                callback(err);
            }
