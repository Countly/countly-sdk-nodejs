 ...
 */
    Countly.init = function(conf) {
        if (!inited) {
            ...
            if (Countly.url === "") {
                log(logLevelEnums.WARNING, "Please provide server URL");
            }
            else {
                log("info", "Countly initialized");
                if (cluster.isMaster) {
    ...
    Countly.group_features = function(features) {
        if (features) {
            for (var i in features) {
                if (!consents[i]) {
                    log(logLevelEnums.INFO, "group_features, Trying to group the features");
                    if (typeof features[i] === "string") {
                        ...
                        log(logLevelEnums.WARNING, "group_features, Incorrect feature list for " + i + " value: " + features[i]);
                    }
                }
                else {
            ...
            log(logLevelEnums.WARNING, "group_features, Incorrect features: " + features);
        }
    };
    ...
    */
    Countly.check_consent = function(feature) {
        ...
        if (consents[feature]) {
            ...
            log(logLevelEnums.WARNING, "check_consent, No feature available for " + feature);
        }
        return false;
    };
    ...
    Countly.check_any_consent = function() {
        ...
        for (var i in consents) {
            if (consents[i] && consents[i].optin) {
                log(logLevelEnums.INFO, "check_any_consent, Consent is given.");

                return true;
            }
        }
        log(logLevelEnums.INFO, "check_any_consent, Consent is not given.");
    ...
    */
    Countly.add_consent = function(feature) {
        log("info", "add_consent, Adding consent for " + feature);
        if (feature.constructor === Array) {
            ...
            log(logLevelEnums.WARNING, "add_consent, No feature available for " + feature);
        }
    };
    ...
    */
    Countly.remove_consent = function(feature) {
        log("info", "remove_consent, Removing consent for " + feature);
    ...
    */
    ...
    Countly.begin_session = function(noHeartBeat) {
        if (Countly.check_consent("sessions")) {
            if (!sessionStarted) {
                log("info", "begin_session, Session started");
    ...
    */
    Countly.session_duration = function(sec) {
        if (Countly.check_consent("sessions")) {
            if (sessionStarted) {
                log("info", "session_duration, Session extended", sec);
    ...
    Countly.end_session = function(sec) {
        if (Countly.check_consent("sessions")) {
            if (sessionStarted) {
                ...
                log("info", "end_session, Ending session");
                reportViewDuration();
                sessionStarted = false;
    ...
    Countly.change_id = function(newId, merge) {
        if (cluster.isMaster) {
            if (Countly.device_id !== newId) {
                ...
                storeSet("cly_id", Countly.device_id);
                log("info", "change_id, Changing ID");
                if (merge) {
    ...
    */
    function add_cly_events(event) {
        if (!event.key) {
            log(logLevelEnums.WARNING, "Event must have key property");
            return;
        }
        if (cluster.isMaster) {
            if (!event.count) {
            ...
            e.dow = date.getDay();
            log(logLevelEnums.DEBUG, "Adding event: ", event);
            eventQueue.push(e);
            storeSet("cly_event", eventQueue);
        }
        else {
    ...
    **/
    Countly.start_event = function(key) {
        if (timedEvents[key]) {
            log(logLevelEnums.INFO, "start_event, Timed event with key " + key + " already started");
            return;
        }
        log(logLevelEnums.INFO, "start_event, Timer for timed event with key " + key + " starting");
    ...
    **/
    Countly.end_event = function(event) {
        ...
        if (!event.key) {
            log(logLevelEnums.WARNING, "end_event, Event must have key property");
            return;
        }
        if (!timedEvents[event.key]) {
            log(logLevelEnums.WARNING, "end_event, Timed event with key " + event.key + " was not started");
            return;
        }
        log(logLevelEnums.INFO, "end_event, Timer for timed event with key " + event.key + " stopping");
    ...
    **/
    Countly.user_details = function(user) {
        if (Countly.check_consent("users")) {
            log(logLevelEnums.DEBUG, "user_details, Adding userdetails: ", user);
    ...
    Countly.report_conversion = function(campaign_id, campaign_user_id) {
        if (Countly.check_consent("attribution")) {
            if (campaign_id && campaign_user_id) {
                ...
                log(logLevelEnums.INFO, "report_conversion, Conversion reported");
                ...
                log(logLevelEnums.WARNING, "report_conversion, No campaign data found");
            }
        }
    };
    ...
    Countly.report_feedback = function(feedback) {
        if (Countly.check_consent("star-rating") || Countly.check_consent("feedback")) {
            if (!feedback.widget_id) {
                log(logLevelEnums.WARNING, "report_feedback, Feedback must contain widget_id property");
                return;
            }
            if (!feedback.rating) {
                log(logLevelEnums.WARNING, "report_feedback, Feedback must contain rating property");
                return;
            }
            if (Countly.check_consent("events")) {
                ...
                log(logLevelEnums.DEBUG, "report_feedback, Reporting feedback: ", event);
                Countly.add_event(event);
            }
        }
    };
    ...
    **/
    Countly.track_errors = function(segments) {
        log(logLevelEnums.INFO, "track_errors, Tracking errors");
    ...
    **/
    Countly.log_error = function(err, segments) {
        log(logLevelEnums.INFO, "log_error, Logging error");
    ...
    Countly.add_log = function(record) {
        if (Countly.check_consent("crashes")) {
            if (crashLogs.length > maxCrashLogs) {
                crashLogs.shift();
                log(logLevelEnums.DEBUG, "add_log, Breadcrumbs overflowed. Erasing the oldest");

            }
            crashLogs.push(record);
            log(logLevelEnums.INFO, "add_log, Added breadcrumb");
        ...
        if (Countly.check_consent("remote-config")) {
            ...
            makeRequest(Countly.url, readPath, request, function(err, params, responseText) {
                try {
                    ...
                    if (cluster.isMaster) {
                        storeSet("cly_remote_configs", remoteConfigs);
                        log(logLevelEnums.INFO, "fetch_remote_config, Fetched remote config");
            ...
            log(logLevelEnums.WARNING, "fetch_remote_config, Remote config requires explicit consent");
            if (typeof callback === "function") {
    ...
    **/
    Countly.get_remote_config = function(key) {
        if (typeof key !== "undefined") {
            log(logLevelEnums.INFO, "get_remote_config, Got remote config key");
            return remoteConfigs[key];

        }
        log(logLevelEnums.INFO, "get_remote_config, Got remote config");
    ...
    **/
    Countly.stop_time = function() {
        log(logLevelEnums.INFO, "stop_time, Stopping time");
    ...
    **/
    Countly.start_time = function() {
        log(logLevelEnums.INFO, "start_time, Starting time");
    ...
    **/
    Countly.track_view = function(name, viewSegments) {
        log(logLevelEnums.INFO, "track_view, Tracking view");
    ...
    **/
        ...
        if (Countly.check_consent("apm")) {
            ...
            for (var i = 0; i < props.length; i++) {
                if (props[i] !== "apm_attr" && typeof trace[props[i]] === "undefined") {
                    log(logLevelEnums.WARNING, "report_trace, APM trace must have a", props[i]);
                    return;
                }
            }
            ...
            log(logLevelEnums.DEBUG, "report_trace, Adding APM trace: ", e);
        }
    };
    ...
    Countly.report_app_start = function() {
        log(logLevelEnums.INFO, "report_app_start, Reporting app start");
    ...
    **/
    Countly.request = function(request) {
        if (!request.app_key || !request.device_id) {
            log(logLevelEnums.WARNING, "request, app_key or device_id is missing");
            return;
        }
        if (cluster.isMaster) {
            requestQueue.push(request);
            storeSet("cly_queue", requestQueue);
            log(logLevelEnums.INFO, "request, Making a raw request");
    ...
    function toRequestQueue(request) {
        if (cluster.isMaster) {
            if (!Countly.app_key || !Countly.device_id) {
                log(logLevelEnums.WARNING, "toRequestQueue, app_key or device_id is missing");
                return;
            }
            prepareRequest(request);
    ...
    function heartBeat() {
        ...
        if (requestQueue.length > 0 && readyToProcess && getTimestamp() > failTimeout) {
            ...
            makeRequest(Countly.url, apiPath, params, function(err, res) {
                log(logLevelEnums.DEBUG, "Request Finished", res, err);
                if (err) {
    ...
    */
    function getMetrics() {
        ...
        log(logLevelEnums.DEBUG, "Got metrics", m);
        return m;
    }
    ...
    function makeRequest(url, api, params, callback) {
        try {
            ...
            req.on("error", function(err) {
                log(logLevelEnums.ERROR, "Connection failed.", err);
                if (typeof callback === "function") {
                    callback(true, params);
                }
            });
            ...
            // fallback
            log(logLevelEnums.ERROR, "Failed HTTP request", e);
            if (typeof callback === "function") {
                callback(true, params);
            }
        }
