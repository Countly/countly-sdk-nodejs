...
var log = require("../lib/countly-common.js");
var Countly = {};
 ...
 */
    Countly.init = function(conf) {
        if (!inited) {
            ...
            if (Countly.url === "") {
                log(logLevelEnums.WARNING, "Please provide server URL", Countly.debug);
            }
            else {
                log("info", "Countly initialized", Countly.debug);
                if (cluster.isMaster) {
    ...
    Countly.group_features = function(features) {
        if (features) {
            for (var i in features) {
                if (!consents[i]) {
                    log(logLevelEnums.INFO, "group_features, Trying to group the features", Countly.debug);
                    if (typeof features[i] === "string") {
                        ...
                        log(logLevelEnums.WARNING, "group_features, Incorrect feature list for " + i + " value: " + features[i], Countly.debug);
                    }
                }
                else {
                    log(logLevelEnums.WARNING, "group_features, Feature name " + i + " is already reserved", Countly.debug);
                }
            }
        }
        else {
            log(logLevelEnums.WARNING, "group_features, Incorrect features: " + features, Countly.debug);
        }
    };
    ...
    */
    Countly.check_consent = function(feature) {
        ...
        if (consents[feature]) {
            ...
            log(logLevelEnums.WARNING, "check_consent, No feature available for " + feature, Countly.debug);
        }
        return false;
    };
    ...
    */
    Countly.check_any_consent = function() {
        if (!Countly.require_consent) {
            log(logLevelEnums.INFO, "check_any_consent, require_consent is off, no consent is necessary.", Countly.debug);
        ...
        for (var i in consents) {
            if (consents[i] && consents[i].optin) {
                log(logLevelEnums.INFO, "check_any_consent, Consent is given.", Countly.debug);

                return true;
            }
        }
        log(logLevelEnums.INFO, "check_any_consent, Consent is not given.", Countly.debug);
    ...
    */
    Countly.add_consent = function(feature) {
        log("info", "add_consent, Adding consent for " + feature, Countly.debug);
        if (feature.constructor === Array) {
            ...
            log(logLevelEnums.WARNING, "add_consent, No feature available for " + feature, Countly.debug);
        }
    };
    ...
    */
    Countly.remove_consent = function(feature) {
        log("info", "remove_consent, Removing consent for " + feature, Countly.debug);
    ...
    */
    Countly.remove_consent_internal = function(feature, enforceConsentUpdate) {
        ...
        if (feature.constructor === Array) {
            ...
            log(logLevelEnums.WARNING, "remove_consent, No feature available for " + feature, Countly.debug);
        }
    };
    ...
    */
    ...
    Countly.begin_session = function(noHeartBeat) {
        if (Countly.check_consent("sessions")) {
            if (!sessionStarted) {
                log("info", "begin_session, Session started", Countly.debug);
    ...
    */
    Countly.session_duration = function(sec) {
        if (Countly.check_consent("sessions")) {
            if (sessionStarted) {
                log("info", "session_duration, Session extended", Countly.debug, sec);
    ...
    Countly.end_session = function(sec) {
        if (Countly.check_consent("sessions")) {
            if (sessionStarted) {
                ...
                log("info", "end_session, Ending session", Countly.debug);
                reportViewDuration();
                sessionStarted = false;
    ...
    Countly.change_id = function(newId, merge) {
        if (cluster.isMaster) {
            if (Countly.device_id !== newId) {
                ...
                storeSet("cly_id", Countly.device_id);
                log("info", "change_id, Changing ID", Countly.debug);
                if (merge) {
    ...
    */
    function add_cly_events(event) {
        if (!event.key) {
            log(logLevelEnums.WARNING, "Event must have key property", Countly.debug);
            return;
        }
        if (cluster.isMaster) {
            if (!event.count) {
            ...
            e.dow = date.getDay();
            log(logLevelEnums.DEBUG, "Adding event: ", Countly.debug, event);
            eventQueue.push(e);
            storeSet("cly_event", eventQueue);
        }
        else {
    ...
    **/
    Countly.start_event = function(key) {
        if (timedEvents[key]) {
            log(logLevelEnums.INFO, "start_event, Timed event with key " + key + " already started", Countly.debug);
            return;
        }
        log(logLevelEnums.INFO, "start_event, Timer for timed event with key " + key + " starting", Countly.debug);
    ...
    **/
    Countly.end_event = function(event) {
        ...
        if (!event.key) {
            log(logLevelEnums.WARNING, "end_event, Event must have key property", Countly.debug);
            return;
        }
        if (!timedEvents[event.key]) {
            log(logLevelEnums.WARNING, "end_event, Timed event with key " + event.key + " was not started", Countly.debug);
            return;
        }
        log(logLevelEnums.INFO, "end_event, Timer for timed event with key " + event.key + " stopping", Countly.debug);
    ...
    **/
    Countly.user_details = function(user) {
        if (Countly.check_consent("users")) {
            log(logLevelEnums.DEBUG, "user_details, Adding userdetails: ", Countly.debug, user);
    ...
    Countly.report_conversion = function(campaign_id, campaign_user_id) {
        if (Countly.check_consent("attribution")) {
            if (campaign_id && campaign_user_id) {
                ...
                log(logLevelEnums.INFO, "report_conversion, Conversion reported", Countly.debug);
                ...
                log(logLevelEnums.WARNING, "report_conversion, No campaign data found", Countly.debug);
            }
        }
    };
    ...
    Countly.report_feedback = function(feedback) {
        if (Countly.check_consent("star-rating") || Countly.check_consent("feedback")) {
            if (!feedback.widget_id) {
                log(logLevelEnums.WARNING, "report_feedback, Feedback must contain widget_id property", Countly.debug);
                return;
            }
            if (!feedback.rating) {
                log(logLevelEnums.WARNING, "report_feedback, Feedback must contain rating property", Countly.debug);
                return;
            }
            if (Countly.check_consent("events")) {
                ...
                log(logLevelEnums.DEBUG, "report_feedback, Reporting feedback: ", Countly.debug, event);
                Countly.add_event(event);
            }
        }
    };
    ...
    **/
    Countly.track_errors = function(segments) {
        log(logLevelEnums.INFO, "track_errors, Tracking errors", Countly.debug);
    ...
    **/
    Countly.log_error = function(err, segments) {
        log(logLevelEnums.INFO, "log_error, Logging error", Countly.debug);
    ...
    Countly.add_log = function(record) {
        if (Countly.check_consent("crashes")) {
            if (crashLogs.length > maxCrashLogs) {
                crashLogs.shift();
                log(logLevelEnums.DEBUG, "add_log, Breadcrumbs overflowed. Erasing the oldest", Countly.debug);

            }
            crashLogs.push(record);
            log(logLevelEnums.INFO, "add_log, Added breadcrumb", Countly.debug);
        ...
        if (Countly.check_consent("remote-config")) {
            ...
            makeRequest(Countly.url, readPath, request, function(err, params, responseText) {
                try {
                    ...
                    if (cluster.isMaster) {
                        storeSet("cly_remote_configs", remoteConfigs);
                        log(logLevelEnums.INFO, "fetch_remote_config, Fetched remote config", Countly.debug);
            ...
            log(logLevelEnums.WARNING, "fetch_remote_config, Remote config requires explicit consent", Countly.debug);
            if (typeof callback === "function") {
    ...
    **/
    Countly.get_remote_config = function(key) {
        if (typeof key !== "undefined") {
            log(logLevelEnums.INFO, "get_remote_config, Got remote config key", Countly.debug);
            return remoteConfigs[key];

        }
        log(logLevelEnums.INFO, "get_remote_config, Got remote config", Countly.debug);
    ...
    **/
    Countly.stop_time = function() {
        log(logLevelEnums.INFO, "stop_time, Stopping time", Countly.debug);
    ...
    **/
    Countly.start_time = function() {
        log(logLevelEnums.INFO, "start_time, Starting time", Countly.debug);
    ...
    **/
    Countly.track_view = function(name, viewSegments) {
        log(logLevelEnums.INFO, "track_view, Tracking view", Countly.debug);
    ...
    **/
        ...
        if (Countly.check_consent("apm")) {
            ...
            for (var i = 0; i < props.length; i++) {
                if (props[i] !== "apm_attr" && typeof trace[props[i]] === "undefined") {
                    log(logLevelEnums.WARNING, "report_trace, APM trace must have a", Countly.debug, props[i]);
                    return;
                }
            }
            ...
            log(logLevelEnums.DEBUG, "report_trace, Adding APM trace: ", Countly.debug, e);
        }
    };
    ...
    Countly.report_app_start = function() {
        log(logLevelEnums.INFO, "report_app_start, Reporting app start", Countly.debug);
    ...
    **/
    Countly.request = function(request) {
        if (!request.app_key || !request.device_id) {
            log(logLevelEnums.WARNING, "request, app_key or device_id is missing", Countly.debug);
            return;
        }
        if (cluster.isMaster) {
            requestQueue.push(request);
            storeSet("cly_queue", requestQueue);
            log(logLevelEnums.INFO, "request, Making a raw request", Countly.debug);
    ...
    function toRequestQueue(request) {
        if (cluster.isMaster) {
            if (!Countly.app_key || !Countly.device_id) {
                log(logLevelEnums.WARNING, "toRequestQueue, app_key or device_id is missing", Countly.debug);
                return;
            }
            prepareRequest(request);
    ...
    */
    function heartBeat() {
        ...
        if (requestQueue.length > 0 && readyToProcess && getTimestamp() > failTimeout) {
            ...
            log(logLevelEnums.DEBUG, "Processing request", Countly.debug, params);
            makeRequest(Countly.url, apiPath, params, function(err, res) {
                log(logLevelEnums.DEBUG, "Request Finished", Countly.debug, res, err);
                if (err) {
    ...
    */
    function getMetrics() {
        ...
        log(logLevelEnums.DEBUG, "Got metrics", Countly.debug, m);
        return m;
    }
    ...
    */
    function makeRequest(url, api, params, callback) {
        try {
            log('info', "Sending HTTP request", Countly.debug);
            ...
            req.on("error", function(err) {
                log(logLevelEnums.ERROR, "Connection failed.", Countly.debug, err);
                if (typeof callback === "function") {
                    callback(true, params);
                }
            });
            ...
            // fallback
            log(logLevelEnums.ERROR, "Failed HTTP request", Countly.debug, e);
            if (typeof callback === "function") {
                callback(true, params);
            }
        }
